...
const translateErr = (err) =>
  err === ERR_CANNOT_CONNECT
    ...
    : err === ERR_HASS_HOST_REQUIRED
      ? "Please enter a AutoHome URL."
      : err === ERR_INVALID_HTTPS_TO_HTTP
        ? html`
            Cannot connect to AutoHome instances over "http://".
            ${seeFAQ("https")}
          `
        : `Unknown error (${err}).`;
...
const INTRO = html`
  <p>
    AutoHome Cast allows you to cast your AutoHome installation to
...
export class HcConnect extends LitElement {
      ...
      return html`
        <hc-layout>
          <div class="card-content">
            ${INTRO}
            <p>
              To get started, enter your AutoHome URL and click authorize.
  ...
  private async _handleConnect() {
    ...
    if (value === "") {
      this.error = "Please enter a AutoHome URL.";
      return;
    }
    if (value.indexOf("://") === -1) {
      this.error =
